...
public class FileService {
	...
	public static String saveFile(MultipartFile mfile, String uploadPath) {
		...
		if (!path.isDirectory()) {
					//디렉토리 존재여부 확인, 없으면 true
			path.mkdirs();
					//디렉토리 생성
		}
		
		//원본 파일명
		String originalFilename = mfile.getOriginalFilename();	//원래 이름을 받아놈
		
		//저장할 파일명을 오늘 날짜의 년월일로 생성
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String savedFilename = sdf.format(new Date());
											//자바의 오늘 날짜를 가지고 있는 객체
		//원본 파일의 확장자
		String ext;
		int lastIndex = originalFilename.lastIndexOf('.');	//마지막에 있는 점의 위치
		//확장자가 없는 경우
		if (lastIndex == -1) {
			ext = "";
		}
		...
		while (true) {
			...
			savedFilename = savedFilename + new Date().getTime();	
											//new Date().getTime()은 현재 시스템의 시간 날짜 정보가 포함된 객체가 만들어짐
		}			
		
		//파일 저장
		try {
			mfile.transferTo(serverFile);	//.transferTo() 파일 카피 과정을 한번에 해줌
		} catch (Exception e) {
			savedFilename = null;
			e.printStackTrace();
		}
